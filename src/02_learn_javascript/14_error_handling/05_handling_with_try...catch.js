...
//
// In the previous exercise we caught an error that we threw, but we can also use a try...catch statement to handle
// built-in errors that are thrown by the JavaScript engine that is reading and evaluating our code.
...
//
// Let’s say we expect to grab an array from a database but the information we get back is a string. In our program, we
// could have a function that only works on arrays. If that function was called with a string instead of an array we
// would get an error and our program would stop running!
//
// However, we can use a try...catch statement to handle the thrown error for us which allows our program to continue
// running and we receive a message knowing what went wrong!
//
