...
class ShiftCipher {
    ...
    encrypt(str = '') {
        ...
        for(let i=0; i<strArr.length; i++) {
            ...
            if((/[a-zA-Z]/).test(char)) {
                if((/[a-zA-Z]/).test(String.fromCharCode(unicodeChar + this.shiftNumber))) {
                    unicodeChar += this.shiftNumber;
                } else {
                    unicodeChar = unicodeChar + this.shiftNumber - 26;
                }
            }

            newStrArr[i] = String.fromCharCode(unicodeChar);
        }
        let newStr = newStrArr.join("").toUpperCase();
        return newStr;
    }
}
